...
INSERT INTO `hca-takehome.reference.dim_appendix_code` (code_sk, code, code_description, code_type, identifier, reference_year)
SELECT TO_HEX(
        SHA256(
...
WHERE NOT EXISTS (
    ...
    WHERE d.code = 'UNKNOWN' AND d.code_type = code_type
);
...
--CREATE row for NULL values for FK references
INSERT INTO `hca-takehome.reference.dim_date` (date_sk, full_date, year, month, year_month, month_start_date)
SELECT -1 AS date_sk,
